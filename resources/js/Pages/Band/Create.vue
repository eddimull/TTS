<template>
    <breeze-authenticated-layout>
        ...
        <div class="w-full max-w-xs">
             <div class="mb-4">
                <div v-if="errors" class="alert alert-danger mt-4">
                    Errors:
                    <ul>
                        <li v-for="(error,prop) in errors" :key="prop">{{ error }}</li>
                    </ul>
                </div>
                <form action="/bands" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" @submit.prevent="createBand">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="name" placeholder="Band Name" v-model="form.site_name">
                    </div>
