        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
          <div class="bg-white dark:bg-gray-700 p-3 rounded shadow">
            ...
            <p class="text-2xl font-bold text-blue-600 dark:text-blue-400">
              {{ getFilteredBookings(band.paidBookings).length }}
            </p>
          </div>
          <div class="bg-white dark:bg-gray-700 p-3 rounded shadow">
            <h4 class="text-sm font-medium text-gray-600 dark:text-gray-300">
            ...
            <p class="text-2xl font-bold text-green-600 dark:text-green-400">
              ${{ getTotalRevenue(getFilteredBookings(band.paidBookings)) }}
            </p>
          </div>
          <div class="bg-white dark:bg-gray-700 p-3 rounded shadow">
            <h4 class="text-sm font-medium text-gray-600 dark:text-gray-300">
            ...
            <p class="text-2xl font-bold text-purple-600 dark:text-purple-400">
              ${{ getTotalPaid(getFilteredBookings(band.paidBookings)) }}
            </p>
          </div>
        </div>
      </div>
...
const filteredServices = computed(() => {
    ...
    if (!selectedYear.value) {
        return props.paid.map((band) => ({
            ...band,
            paidBookings: band.paidBookings.map(booking => ({
                ...booking,
    ...
    return props.paid.map((band) => ({
        ...band,
        paidBookings: band.paidBookings
            .filter((paid) => {
                // Handle year filter - check if the date string contains the selected year
                return paid.date.includes(selectedYear.value.toString());
            })
            .map(booking => ({
                ...booking,
                date: new Date(booking.date) // Ensure date is a Date object for proper filtering
