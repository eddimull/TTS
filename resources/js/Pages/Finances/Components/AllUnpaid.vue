...
const filteredServices = computed(() => {
  ...
  return props.unpaid.map(band => ({
    ...
    unpaidBookings: band.unpaidBookings.filter(unpaid =>
      unpaid.amount_paid.toString().toLowerCase().includes(serviceFilter.value.toLowerCase()) ||
      unpaid.name.toLowerCase().includes(serviceFilter.value.toLowerCase()) ||
      unpaid.price.toString().includes(serviceFilter.value) ||
      unpaid.date.toLowerCase().includes(serviceFilter.value.toLowerCase())
    )
  }));
});
