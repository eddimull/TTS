...
export default {
  ...
  computed: {
    ...
    sessionStatus() {
      ...
      return { active: 'Live', paused: 'Paused', completed: 'Completed', pending: 'Pending' }[this.localSession.status] ?? this.localSession.status;
    },

    statusSeverity() {
      if (!this.localSession) return 'secondary';
      return { active: 'danger', paused: 'warn', completed: 'success', pending: 'info' }[this.localSession.status] ?? 'secondary';
    },
  ...
  mounted() {
    if (this.localSession) {
      ...
      if (this.isDynamic && !this.currentSong) {
        this.aiMode = true;
