<template>
  <div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div
      v-if="(booking.status === 'confirmed' || booking.status === 'pending') && (booking.contract_option === 'default' || booking.contract_option === 'external')"
      class="space-y-6"
    >
      <p
        v-if="booking.status === 'confirmed' && booking.contract_option === 'default'"
        class="text-xl text-center text-gray-700 font-semibold bg-yellow-100 py-3 px-4 rounded-lg shadow-sm"
      >
        This contract is confirmed. The contract is no longer editable.
      </p>
      <p
        v-if="booking.status === 'confirmed' && booking.contract_option === 'external'"
        class="text-xl text-center text-gray-700 font-semibold bg-yellow-100 py-3 px-4 rounded-lg shadow-sm"
      >
      ...
      <div class="grid gap-6 lg:grid-cols-3">
        <div
          ...
          :class="[booking.contract_option === 'default' ? 'lg:col-span-2' : 'lg:col-span-3']"
        >
          <h2 class="text-lg font-semibold bg-gray-100 dark:bg-slate-700 px-4 py-2 border-b">
            Contract Preview
          </h2>
          <iframe
