...
<template>
  <div class="mt-4 p-4 bg-gray-100 rounded-lg">
    ...
    <div class="mt-4">
      ...
      <textarea v-model="event.notes" class="w-full p-2 border rounded" rows="3"></textarea>
    </div>
    
    <!-- Times Section -->
    <div class="mt-4">
      <h3 class="text-xl font-semibold mb-2">Times</h3>
      <div v-for="(entry, index) in timeEntries" :key="index" class="flex items-center mb-2">
        <input
        ...
        />
        <select v-model="entry.isLoadIn" class="w-1/4 p-2 border rounded mr-2">
          <option :value="false">Regular Time</option>
          <option :value="true">Load-in Time</option>
        </select>
        <input
          v-model="entry.value"
          type="datetime-local"
        ...
        />
        <button @click="removeTimeEntry(index)" class="px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600">
          Remove
        </button>
      </div>
      <div class="mt-2">
        <button @click="addTimeEntry(false)" class="mr-2 px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600">
          Add Regular Time
        </button>
        <button @click="addTimeEntry(true)" class="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">
          Add Load-in Time
        </button>
      </div>
    </div>
    ...
    <div class="mt-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <template v-for="(value, key) in event.additional_data" :key="key">
          <template v-if="key !== 'dances' && key !== 'onsite' && key !== 'times'">
            <div v-if="typeof value === 'object' && value !== null">
              <h4 class="font-semibold mb-2">{{ formatLabel(key) }}</h4>
    ...
    <div v-if="isWedding" class="mt-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <div v-for="(value, key) in event.additional_data.dances" :key="key" class="mb-2">
