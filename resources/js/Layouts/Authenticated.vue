        ...
        <nav class="fixed z-50 w-full bg-white border-b border-gray-100">
                ...
                <div class="flex justify-between h-16">
                    <div class="flex">
                        ...
                        <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                            ...
                            <breeze-nav-link v-if="navigation && navigation.Bookings" :href="route('Bookings Home')"
                                :active="route().current('Bookings Home')">
                                Booking
                            </breeze-nav-link>
                            <breeze-nav-link v-if="navigation && navigation.Events" :href="route('events')"
                                :active="route().current('events')">
                                Events
                            </breeze-nav-link>
                            <breeze-nav-link v-if="navigation && navigation.Proposals" :href="route('proposals')"
                                :active="route().current('proposals')">
                                Proposals
                            </breeze-nav-link>
                            <breeze-nav-link v-if="navigation && navigation.Invoices" :href="route('finances')"
                                :active="route().current('finances')">
                                Finances
                            </breeze-nav-link>
                            <breeze-nav-link v-if="navigation && navigation.Colors" :href="route('colors')"
                                :active="route().current('colors')">
                                Colors
                            </breeze-nav-link>
                            <breeze-nav-link v-if="navigation && navigation.Charts" :href="route('charts')"
                                ...
                                <template #trigger>
                                    <span class="inline-flex rounded-md">
                                        <button type="button"
                                            ...
                                            @click="markSeen">
                                            <span v-if="unseenNotifications > 0"
                                ...
                                <template #content>
                                    <div class="flex flex-col max-h-72">
                                        <div class="overflow-y-auto flex-auto">
                                            <notification-link v-for="(notification, index) in notifications"
                                                :key="index" :unread="notification.read_at === null"
                                                ...
                                                method="get" as="button" @click="markAsRead(notification)">
                        ...
                        <breeze-dropdown align="full" width="full">
                            ...
                            <template #content>
                                <div class="flex flex-col flex-grow max-h-72">
                                    <div class="overflow-y-auto flex-auto">
                                        <notification-link v-for="(notification, index) in notifications" :key="index"
            ...
            <div :class="{ 'block': showingNavigationDropdown, 'hidden': !showingNavigationDropdown }"
                ...
                <div class="pt-2 pb-3 space-y-1">
                    ...
                    <breeze-responsive-nav-link v-if="navigation && navigation.Events" :href="route('events')"
                        :active="route().current('events')">
                        Events
                    </breeze-responsive-nav-link>
                    <breeze-responsive-nav-link v-if="navigation && navigation.Proposals" :href="route('proposals')"
                        :active="route().current('proposals')">
                        Proposals
                    </breeze-responsive-nav-link>
                    <breeze-responsive-nav-link v-if="navigation && navigation.Bookings" :href="route('Bookings Home')"
                        :active="route().current('Bookings Home')">
                        Booking
                    </breeze-responsive-nav-link>
                    <breeze-responsive-nav-link v-if="navigation && navigation.Events" class="pl-4"
                        :href="route('questionnaire')" :active="route().current('questionnaire')">
                        Questionnaires
                    </breeze-responsive-nav-link>
                    <breeze-responsive-nav-link v-if="navigation && navigation.Invoices" :href="route('finances')"
                        :active="route().current('finances')">
                        Finances
                    </breeze-responsive-nav-link>
                    <breeze-responsive-nav-link v-if="navigation && navigation.Colors" :href="route('colors')"
                        :active="route().current('colors')">
                        Colors
                    </breeze-responsive-nav-link>

                    <breeze-responsive-nav-link v-if="navigation && navigation.Charts" :href="route('charts')"
                        :active="route().current('charts')">
                        Charts
                    </breeze-responsive-nav-link>
                </div>
...
export default {
    ...
    methods: {
        ...
        toast() {
            console.log('this.$page.props.errors', this.$page.props)
